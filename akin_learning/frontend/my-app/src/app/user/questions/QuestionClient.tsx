...
// ----------------------
export default function QuestionsClient({userData, questions: initialQuestions}: QuestionClientProps) {
    ...
    //OLD LOCAL HANDLER FOR QUESTIONS
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await fetch(
                `http://localhost:5000/questions/${currentQuestion.id}/answer`,
                {
                    method: "POST",
                    headers: {"Content-Type": "application/json"},
                    body: JSON.stringify({selected_option_id: selectedOptionId, user_id: userData.id}),
                }
            );
    ...
    const handleChatbotMessageSubmit = async (message: string) => {
        ...
        try {
            const res = await fetch("http://localhost:5000/api/openai/fetch", {
                method: "POST",
                headers: {"Content-Type": "application/json"},
