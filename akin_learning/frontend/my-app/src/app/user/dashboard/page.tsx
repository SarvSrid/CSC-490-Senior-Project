...
interface ProgressData {
  ...
  last_visited_question_id?: number | null; // Add this
}
  ...
  useEffect(() => {
    ...
    if (!userData) {
      ...
      Promise.all([
        ...
        fetch(`http://localhost:5003/api/user-progress`)
      ])
      .then(async ([progressRes, userProgressRes]) => {
        const progressData = await progressRes.json();
        const userProgress = await userProgressRes.json();
        
        const mergedData = progressData.map((item: any) => ({
          ...item,
          last_visited_question_id: userProgress.find(
            (up: any) => up.topic_id === item.subject_id
          )?.last_visited_question_id
        }));
            ...
            <div key={index} className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Progress: {Math.round(item.average_progress)}%
                </p>
              </div>
              <Link href={`/user/topics?subject_id=${item.subject_id}`}>
                <button
